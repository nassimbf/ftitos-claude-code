...
**REVIEWS.md** (if included by reviews mode) — Cross-AI review feedback from `/review`
...
@references/few-shot-examples/plan-checker.md
...
**Question:** Do plans honor user decisions from /discuss-phase?
...
their skip condition and the Dimension 8 output table: @references/nyquist-compliance.md
...
@references/failing-direction.md
...
`{VERIFY_PATHS}` probe, never re-run/hand-reason it: @references/verify-command-path-resolvability.md
...
@references/plan-checker-examples.md
